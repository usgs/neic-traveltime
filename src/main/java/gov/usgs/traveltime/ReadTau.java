...
public class ReadTau {
  ...
  public void readHeader(String hedFileName) throws IOException {
    ...
    bytesRead = in.read(byteArray, byteBuf.position(), byteArray.length - byteBuf.position());
  ...
  public void dumpRec1(boolean full) {
    ...
    System.out.println(
        "numMod numSeg numBrn numTauUp numXUp = "
            + numMod[0]
            + " "
            + numMod[1]
            + " "
    ...
    if (full) {
      ...
      for (int j = 0; j < typeSeg.length; j++)
        System.out.println(
            "" + j + ": " + typeSeg[j][0] + ", " + typeSeg[j][1] + ", " + typeSeg[j][2]);

      System.out.println("\nindexSeg:");
      for (int j = 0; j < indexSeg.length; j++)
        System.out.println("" + j + ": " + indexSeg[j][0] + ", " + indexSeg[j][1]);
    ...
    System.out.println(
        "\nrUpperMantle RMoho rConrad = " + rUpperMantle + " " + rMoho + " " + rConrad);
  }
  ...
  public void dumpRec4() {
    ...
    System.out.println("\npBrn:");
    for (int j = 0; j < pBrn.length; j++)
      System.out.println("" + j + ": " + (float) pBrn[j][0] + ", " + (float) pBrn[j][1]);

    System.out.println("\nxBrn:");
    for (int j = 0; j < xBrn.length; j++)
      System.out.println("" + j + ": " + (float) xBrn[j][0] + ", " + (float) xBrn[j][1]);
  ...
  public void dumpGlobal() {
    System.out.println("\n     Global Varaiables");
    System.out.format(
        "Limits: recSizeUp =%5d  numSpec =%5d  numSeg =%3d  " + "numBrn =%3d\n",
        recSizeUp, numSpec, numSeg, numBrn);
    System.out.format(
        "Normalization: xNorm =%11.4e  pNorm =%11.4e  " + "tNorm =%11.4e\n", xNorm, pNorm, tNorm);
    System.out.format(
        "Radii: rUpperMantle =%7.1f  RMoho =%7.1f  rConrad =%7.1f\n", rUpperMantle, rMoho, rConrad);
  }
  ...
  public void dumpModel(boolean nice) {
    ...
    if (nice) {
      for (int j = 0; j < numMod[0]; j++) {
        System.out.format(
            "%3d: %6.1f  %5.2f  %3d  %6.1f %5.2f  %3d\n",
            j,
            realZ(zMod[0][j]),
            realV(pMod[0][j], zMod[0][j]),
            indexMod[0][j],
            realZ(zMod[1][j]),
      ...
      for (int j = numMod[1]; j < numMod[1] + 3; j++) {
        System.out.format(
            "%3d:                     %6.1f %5.2f\n",
            j, realZ(zMod[1][j]), realV(pMod[1][j], zMod[1][j]));
      }
    } else {
      for (int j = 0; j < numMod[0]; j++) {
        System.out.format(
