...
public class UpGoingDataReference implements Serializable {
   ...
   */
  public UpGoingDataReference(ReadTau in, char typeUp) {
    this.typeUp = typeUp;
    int i = -1, k;
   ...
   * @param typeUp Wave type ('P' or 'S')
   */
  public UpGoingDataReference(TauModel finModel, char typeUp) {
    int n, k = -1;
  ...
  public void dumpUp(ModelDataReference model, ModelConversions convert) {
    for (int rec = 0; rec < tauUp.length; rec++) {
      System.out.format(
          ...
          typeUp, rec, convert.computeDimensionalDepth(model.getUpGoingDepth(rec)));
      System.out.println("          p        tau        p           X");
      for (int k = 0; k < xUp[rec].length; k++) {
        System.out.format(
            "%3d  %8.6f  %8.6f  %8.6f  %9.6f\n", k, pTauUp[k], tauUp[rec][k], pXUp[k], xUp[rec][k]);
