...
/**
 * The TTime class contains the complete information (including auxiliary information) for all
...
public class TTime {
  ...
  /** An ArrayList of TTimeData objects containing for each phase */
  private ArrayList<TTimeData> phaseTravelTimes;

  /** Private logging object. */
  private static final Logger LOGGER = Logger.getLogger(TTime.class.getName());

  /**
   * The constructor for the TTime class. This constructor allocates an ArrayList to hold the phase
   * data, and saves the depth and source-receiver distance for convenience.
   *
   ...
   */
  public TTime(double depth, double distance) {
    this.depth = depth;
    this.distance = distance;

    phaseTravelTimes = new ArrayList<TTimeData>();
  }
  ...
  public void finalizeTravelTimes(boolean convertTectonic, boolean returnBackBranches) {
    ...
    phaseTravelTimes.sort(new ArrComp());

    // Filter the phases to make them easier to use.
    if (convertTectonic) {
      TauUtil.filterTect(phaseTravelTimes);
    }

    if (!returnBackBranches) {
      TauUtil.filterBack(phaseTravelTimes);
    } else {
      TauUtil.filterDef(phaseTravelTimes);
    }

    // A catch all filter to compensate for strange model artifacts.
    TauUtil.filterMisc(phaseTravelTimes, distance);

    // Modify the observabilities of phases closely following
    // another phase in time.
    TauUtil.modObserv(phaseTravelTimes);

    // Modify canUse for phases with no statistics.
    TauUtil.modCanUse(phaseTravelTimes);
  }
   ...
   * @return TTimeData object containing the Travel-time data
   */
  public TTimeData getPhase(int index) {
