...
public class AuxTtRef {
  ...
  public AuxTtRef(boolean readStats, boolean readEllip, boolean readTopo, String modelPath)
    ...
    if (FileChanged.isChanged(TauUtil.model(serName), absNames)) {
      ...
      inEllip = new BufferedInputStream(new FileInputStream(absNames[2]));
  ...
  private PhGroup read1Group() {
    if (nextCode.contains(":")) {
      PhGroup group = new PhGroup(nextCode.substring(0, nextCode.indexOf(':')));
      nextCode = scan.next();
   ...
   */
  public double getSpread(TtStat ttStat, double delta, boolean upGoing) {
   ...
   */
  public double getObserv(TtStat ttStat, double delta, boolean upGoing) {
  ...
  private void unTangle(String phCode, String phGroup) {
    ...
    if ((phGroup.equals("P") || phGroup.equals("S")) && !phCode.contains("dif")) {
