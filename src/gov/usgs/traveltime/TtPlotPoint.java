 ...
 *
 */
public class TtPlotPoint implements Comparable<TtPlotPoint> {
	 ...
	 */
	public TtPlotPoint(double delta, double tt, double spread, double observ, 
			double dTdD) {
		this.delta = delta;
		this.tt = tt;
		if(spread < TauUtil.DEFSPREAD) {
			this.spread = spread;
			this.observ = observ;
		} else {
	...
	/**
	 * We need to be able to sort the plot points into ray parameter 
	 * order so caustics will look OK.
	 */
	@Override
