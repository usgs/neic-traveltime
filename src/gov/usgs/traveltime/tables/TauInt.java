...
public class TauInt {
	...
	public double intX(char type, double p, int limit) throws Exception {
		...
		for(j=tabModel.size()-1; j>limit; j--) {
			...
			tauSum += intLayer(p, tabModel.getSlow(type, j), tabModel.getSlow(type, j-1), 
...
//				tabModel.getSlow(type, j), p, tabModel.getZ(j), zLast, x);
	...
	/**
	 * Compute the derivative of ray travel distance with respect to ray 
	 * parameter for one mode layer.
	 * 
	 * @param p Normalized ray parameter
	 * @param pTop Normalized slowness at the top of the layer
	 * @param pBot Normalized slowness at the bottom of the layer
	 ...
	 */
	public double intDeriv(double p, double pTop, double pBot, 
			double zTop, double zBot) {
		double p2, pTop2, pBot2;
		
		// Handle a zero thickness layer (discontinuity) or a constant 
		// slowness layer.
		if(Math.abs(zTop-zBot) <= TauUtil.DTOL || Math.abs(pTop-pBot) <= 
				TauUtil.DTOL) {
			return 0d;
		}
