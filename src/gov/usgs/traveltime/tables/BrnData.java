...
public class BrnData {
		...
		if(p == null) {
			...
			if(!nice) {
				...
				if(full) {
					...
					if(x != null) {
						for(int j=0; j<p.length; j++) {
							System.out.format("%3d: %8.6f %8.6f %8.6f %9.2e %9.2e %9.2e %9.2e "+
									"%9.2e\n", j, p[j], tau[j], x[j], basis[0][j], basis[1][j], 
									basis[2][j], basis[3][j], basis[4][j]);
						}
					} else {
						for(int j=0; j<p.length; j++) {
							System.out.format("%3d: %8.6f %8.6f %8.6f %9.2e %9.2e %9.2e %9.2e "+
									"%9.2e\n", j, p[j], 0d, 0d, basis[0][j], basis[1][j], 
									basis[2][j], basis[3][j], basis[4][j]);
						}
					}
				}
			} else {
				System.out.format("%-8s %2d %c %c %c %1d %5b %8.6f %8.6f %6.2f %6.2f %3d\n", 
						phCode, signSeg, typeSeg[0], typeSeg[1], typeSeg[2], countSeg, 
				...
				if(full) {
					...
					if(x != null) {
						for(int j=0; j<p.length; j++) {
							System.out.format("%3d: %8.6f %8.6f %6.2f %9.2e %9.2e %9.2e %9.2e "+
									"%9.2e\n", j, p[j], tau[j], Math.toDegrees(x[j]), basis[0][j], 
									basis[1][j], basis[2][j], basis[3][j], basis[4][j]);
						}
					} else {
						for(int j=0; j<p.length; j++) {
							System.out.format("%3d: %8.6f %8.6f %6.2f %9.2e %9.2e %9.2e %9.2e "+
									"%9.2e\n", j, p[j], 0d, 0d, basis[0][j], basis[1][j], basis[2][j], 
									basis[3][j], basis[4][j]);
						}
					}
				}
	...
	public String toString() {
		if(p == null) {
			return String.format("%-8s %2d %c %c %c %1d %5b", phCode, signSeg, 
					typeSeg[0], typeSeg[1], typeSeg[2], countSeg, isUpGoing);
		} else {
			return String.format("%-8s %2d %c %c %c %1d %5b %8.6f %8.6f %8.6f %8.6f %3d", 
					phCode, signSeg, typeSeg[0], typeSeg[1], typeSeg[2], countSeg, isUpGoing, 
					pRange[0], pRange[1], xRange[0], xRange[1], p.length);
		}
	}
}
